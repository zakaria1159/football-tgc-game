...
T.test("result ribbons are colour-coded by outcome", function()
    ...
    T.eq(Fx.result("defender_destroyed", 300, true).text, "DESTROYED", "a tackle shows no LP")
end)
...
T.test("a tackle record gets a TACKLE tag; other fights don't", function()
    T.eq(Fx.tackleTag({ outcome = "defender_destroyed", tackle = true }), "TACKLE")
    T.eq(Fx.tackleTag({ outcome = "attacker_exhausted", tackle = true }), "TACKLE")
    T.eq(Fx.tackleTag({ outcome = "defender_destroyed" }), nil)
    T.eq(Fx.tackleTag(nil), nil)
end)
