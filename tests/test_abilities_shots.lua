...
T.test("Fortress: no trigger when the bonus is 0 (no active cards in front)", function()
    local m = H.match()
    H.place(m, "player", "striker", 1, H.card("striker", 2300, 500))
    H.place(m, "opponent", "keeper", 0, H.kw("FORTRESS", "keeper", 300, 2000))
    local s = H.store(m)
    local card = H.give(m, "player", H.def("strat-penalty"))
    local r = s:playStrategy(card.id)
    T.eq(r.outcome, "damage"); T.eq(r.damage, 300)                -- 2300 vs 2000, no line bonus
    T.eq(s.combatQueue[1].defender.def, 2000)
    T.eq(#H.triggers(m), 0)
end)
